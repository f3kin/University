...
/**
* Function created to empty and free the memory of the heap
*/
void emptyPQ(struct heap* pq, int lines) {
	while(pq->count != 0) {
		node_t* n = heap_delete(pq);
		//added a call to free the state memory of the nodes
		free_state(n->state, lines);
		free(n);
...
/**
* This is a function created to free the state pointer of each node to properly free the node
* It frees every line of the map first and then the map pointer itself, so no memory is lost
*/
void free_state(state_t current_state, int lines) {
	for(int i = 0; i < lines; i++) {
		// Free each line of the map
		free(current_state.map[i]);
	}
	// Free the map itself
